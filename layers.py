...
def matrix_attention_layer(input_ts, context_ts, att_dim, scope_name, weights_only=False):
    ...
    with tf.variable_scope(scope_name):
        ...
        attention_input = tf.tanh(layer_utils.tensor_matmul(input_ts, w_i) + layer_utils.tensor_matmul(tiled_context, w_c) + b_i)
        ...
        attention_score = tf.nn.softmax(tf.squeeze(layer_utils.tensor_matmul(attention_input, w_a), axis=[2]))
        if weights_only:
            return attention_score
        #attention_score = tf.nn.softmax(layer_utils.tensor_matmul(attention_input, w_a))
...
def bilinear_attention_layer(input_ts, context_ts, att_dim, scope_name):
    ...
    with tf.variable_scope(scope_name):
        ...
        attention_input = tf.matmul(layer_utils.tensor_matmul(input_ts, p), reshaped_context)
        #print 'attention_input:', attention_input.shape
        # (batch_size, time_step)
